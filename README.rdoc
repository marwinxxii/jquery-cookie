...
other variants: '1y' - one year, '1m' - one month, '1h' - one hour

Create expiring cookie, valid across entire page:

    $.cookie('the_cookie', 'the_value', { expires: '7d', path: '/' });
...
Define lifetime of the cookie. Value can be a +Number+ (which will be interpreted as milliseconds from Unix epoch), or +String+ or a +Date+ object. If omitted, the cookie is a session cookie.
